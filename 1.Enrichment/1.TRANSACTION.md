...
This document specifically describes the `transaction` event.
...
```jsonc
{
    ...
    "data": {
        ...
        // external fields
        "externalTransactionIdentifier": "123456789", // external TID
        "orderNumber": "123456789", // external order number

        // tip: same as sent by the brand
        "entryMode": "5",
        "electronicCommerceIndicator": "05", // ECI - never null, use "" if not available
...
| `data.authorizationDate` | string | Yes | Date and time of authorization in RFC 3339 format |
...
| `data.transactionType` | string | Yes | Transaction type. Values: "debit", "credit", "prepaid" |
| `data.entryMode` | string | Yes | Card entry mode (same as sent by the card brand) |
| `data.electronicCommerceIndicator` | string | No | Electronic Commerce Indicator (ECI). Never null, use `""` if not available |
| `data.issuer` | object | No | Card issuer information |
| `data.issuer.code` | string | No | Card issuer code |
| `data.issuer.name` | string | No | Card issuer name |
...
| `data.externalTransactionIdentifier` | string | No | External transaction identifier (TID)|
| `data.orderNumber` | string | No | External order number |
...
| `installmentType` | string | No | Installment type. Required when installment data is provided. Values: "issuer", "acquirer" |
| `acquirerIssuerProcessingDate` | string | Yes | Clearing record processing date in RFC 3339 format |
| `acquirerIssuerExpectedSettlementDate` | string | Yes | Expected date for the Issuer to settle the transaction to the Acquirer, in RFC 3339 format |
| `payoutExpectedSettlementDate` | string | No | Expected date for the Acquirer or Subacquirer to settle the transaction to their represented party (Merchant or Subacquirer), in RFC 3339 format |
...
| `voucherProcessingDate` | string | Yes | Voucher processing date in RFC 3339 format |
