...
Este documento descreve especificamente o evento `transaction`.
...
```jsonc
{
    ...
    "data": {
        ...
        "orderNumber": "123456789", // numero de pedido externo

        // dica: mesmo que enviado pela bandeira
        "entryMode": "5",
        "electronicCommerceIndicator": "05", // ECI - nunca nulo, usar "" se não disponível
...
| `data.authorizationDate` | string | Sim | Data e hora da autorização no formato RFC 3339 |
...
| `data.transactionType` | string | Sim | Tipo da transação. Valores: "debit", "credit", "prepaid" |
| `data.entryMode` | string | Sim | Modo de entrada do cartão (mesmo enviado pela bandeira do cartão) |
| `data.electronicCommerceIndicator` | string | Não | Indicador de Comércio Eletrônico (ECI). Nunca nulo, usar `""` se não disponível |
| `data.issuer` | object | Não | Informações do emissor do cartão |
| `data.issuer.code` | string | Não | Código do emissor do cartão |
| `data.issuer.name` | string | Não | Nome do emissor do cartão |
...
| `data.externalTransactionIdentifier` | string | Não | Identificador de transação externo (TID) |
| `data.orderNumber` | string | Não | Número de pedido externo |
...
| `installmentType` | string | Não | Tipo da parcela. Obrigatório quando dados de parcelamento são informados. Valores: "issuer", "acquirer" |
| `acquirerIssuerProcessingDate` | string | Sim | Data de processamento do clearing record no formato RFC 3339 |
| `acquirerIssuerExpectedSettlementDate` | string | Sim | Data prevista para o Issuer liquidar a transação para o Adquirente, no formato RFC 3339 |
| `payoutExpectedSettlementDate` | string | Não | Data prevista para o Adquirente ou Subadquirente liquidar a transação para seu representado (Merchant ou Subadquirente), no formato RFC 3339 |
| `amount` | number | Sim | Valor da parcela em centavos |
| `acquirerIssuerSettlementStatus` | string | Sim | Status de liquidação com o emissor. Valores: "settled", "scheduled", "canceled", "confirmed", "sent" |
| `acquirerMerchantSettlementStatus` | string | Sim | Status de liquidação com o comerciante. Valores: "settled", "scheduled", "canceled" |
...
| `voucherProcessingDate` | string | Sim | Data de processamento do voucher no formato RFC 3339 |
