...
This document specifically describes the `merchant` event.
...
| `payload.transactionIdentifier` | string | No | Acquirer's unique identifier for the merchant's transaction |
...
| `merchantCategoryCode` | string | No | MCC (Merchant Category Code) identifying the merchant's business type. Must match a code registered in the platform |
| `treatmentCategory` | string | Yes | Merchant treatment tier defined by the client. Must match one of the categories configured in the platform (e.g. `gold`, `silver`, `plus`, `standard`) |
| `guaranteedChargeback` | boolean | No | Indicates whether the client has chargeback guarantee and the partner (sub-acquirer or acquirer) assumes that chargeback |
