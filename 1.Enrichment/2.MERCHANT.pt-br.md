...
| `merchantCategoryCode` | string | Não | MCC (Código de Categoria do Comerciante) que identifica o tipo de negócio do comerciante. Deve corresponder a um código cadastrado na plataforma |
| `treatmentCategory` | string | Sim | Categoria de tratamento do comerciante definida pelo cliente. Deve corresponder a uma das categorias configuradas na plataforma (ex: `gold`, `silver`, `plus`, `standard`) |
| `guaranteedChargeback` | boolean | Não | Indica se o cliente possui garantia de chargeback e se o parceiro (sub-adquirente ou adquirente) assume aquele chargeback |
